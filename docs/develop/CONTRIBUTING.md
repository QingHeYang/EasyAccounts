...
- **本项目仅接受 Bug 修复类 PR，暂不接受新功能 PR**。新功能涉及产品定位、用户体验、数据模型一致性等多方面取舍，需要从全局规划，避免破坏性升级影响已有用户。有功能想法请先在 [Issue](https://github.com/QingHeYang/EasyAccounts/issues) 区讨论。
- 项目严禁修改`main`分支。
...
{% hint style="info" %}
**关于源码同步**：源码仓库相对发布版本会延迟一段时间——新版本通常包含较多结构调整，需要先在内部跑稳再开源，避免半成品代码误导贡献者。请耐心等待源码仓库同步到最新版本再贡献。
{% endhint %}
